<?php
class ControllerSaleOrderCC extends Controller {
	private $error = array();
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			...
			$this->redirect($this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['invoice'] = $this->url->link('sale/orderCC/invoice', 'format=raw&tmpl=component&token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('sale/orderCC/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/orderCC/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/orderCC/update', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
			*/
		...
		$this->data['sort_order'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$this->data['sort_customer'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$this->data['sort_total'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		$this->data['sort_date_modified'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . '&sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'sale/order_listCC.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	public function getForm() {
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'], 'SSL'),				
			'separator' => ' :: '
		);

		if (!isset($this->request->get['order_id'])) {
			$this->data['action'] = $this->url->link('sale/orderCC/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('sale/orderCC/update', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['card_name'])) {
			...
			$this->data['card_name'] = $order_info['payment_name'];
		} else {
      		$this->data['card_name'] = '';
    	}
		...
		if (isset($this->request->post['card_type'])) {
			...
			$this->data['card_type'] = $order_info['payment_card_type'];
		} else {
      		$this->data['card_type'] = '';
    	}
		
		if (isset($this->request->post['payment_cc'])) {
      		$this->data['payment_cc'] = $this->request->post['payment_cc'];
    	} elseif (!empty($order_info)) { 
		
			$key = $this->config->get('offline_encryption');
			$decrypted = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($key), base64_decode($order_info['payment_cc']), MCRYPT_MODE_CBC, md5(md5($key))), "\0");
			
			$this->data['payment_cc'] = $decrypted;
		} else {
      		$this->data['payment_cc'] = '';
    	}
	...
	public function info() {
		...
		$order_info = $this->model_sale_orderCC->getOrder($order_id);
		...
		if ($order_info) {
			...
			$this->data['invoice'] = $this->url->link('sale/orderCC/invoice', 'format=raw&tmpl=component&token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['cancel'] = $this->url->link('sale/orderCC', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			if(isset($order_info['payment_card_type']) ) {
				$this->data['card_type'] = $order_info['payment_card_type'];
			}
			
			if(isset($order_info['payment_name']) ) {
				$this->data['card_name'] = $order_info['payment_name'];
			}
			...
			$products = $this->model_sale_orderCC->getOrderProducts($this->request->get['order_id']);

			foreach ($products as $product) {
				$option_data = array();

				$options = $this->model_sale_orderCC->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						...
						$option_data[] = array(
							...
							'type'  => $option['type'],
							'href'  => $this->url->link('sale/orderCC/download', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&order_option_id=' . $option['order_option_id'], 'SSL')
						);						
					}
				}
			...
			$this->data['totals'] = $this->model_sale_orderCC->getOrderTotals($this->request->get['order_id']);

			$this->data['downloads'] = array();

			$results = $this->model_sale_orderCC->getOrderDownloads($this->request->get['order_id']);
			...
			$this->template = 'sale/order_infoCC.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'sale/orderCC')) {
	...
	public function createInvoiceNo() {
		$this->language->load('sale/orderCC');
	...
	public function history() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/orderCC')) { 
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/orderCC/history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
